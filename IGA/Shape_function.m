        ...
        for i = 0 : p
          ...
          if e>432&&k==0
            R(loc_num) = N(p+1-i)*M(q+1-j)*L(r+1-k) ...
                        * B{ni-i,nj-j,1}(4); % Function numerator (N*M*L*w) 
            x(loc_num) = B{ni-i,nj-j,1}(1) ;
            y(loc_num) = B{ni-i,nj-j,1}(2);
            z(loc_num) = B{ni-i,nj-j,1}(3);
             dR_dxi(loc_num,1) = dN_dxi(p+1-i)*M(q+1-j)*L(r+1-k) ...
           * B{ni-i,nj-j,1}(4); % Derivative numerator (dN*M*L*w)
        
            dR_dxi(loc_num,2) = N(p+1-i)*dM_deta(q+1-j)*L(r+1-k) ...
            * B{ni-i,nj-j,1}(4); % Derivative numerator (N*dM*L*w)


           dR_dxi(loc_num,3) = N(p+1-i)*M(q+1-j)*dL_dzeta(r+1-k) ...
           * B{ni-i,nj-j,1}(4); % Derivative numerator (N*M*dL*w)
            else
           R(loc_num) = N(p+1-i)*M(q+1-j)*L(r+1-k) ...
